...
int main() {
    ...
    glGenTextures(1, &texture); // creatures and stores a texture into an array, similar to vertex buffers
    glBindTexture(GL_TEXTURE_2D, texture); // binds the texture so it can be configured
    ...
    unsigned char* data = stbi_load("PIERRE WHAT.png", &width, &height, &nrChannels, 0); // loads image data and stores its dimensions
